    ...
    <div>
         <div className="card  w-full mt-20 h-fit lg:p-6 p-3 shadow-2xl lg:ml-6">
                ...
                <input type="file"   id="file-upload" name="selectedFile"
                  ...
                  <h4 className='font-semibold text-black-100 lg:text-2xl md:text-xl text-sm pt-6 capitalize'>{projectOwner}</h4>
                    <p className='font-medium text-black-100 pt-4 lg:text-base text-sm '>Project Founder</p>
                </div>
                <div className="card-body  w-full flex flex-col mt-6 pl-6 gap-0 border-custom-brown border ">
                  <p  className="lg:text-lg text-base  font-medium  mb-4 text-black-100 flex flex-wrap items-center ">
                  ...
                  {backers?.length > 0? backers.map((backer)=>{
                    return ( <div key={backer.id} className='flex flex-wrap items-center pt-6'>
                    <span className='mr-6'><TbCurrencyNaira /></span>
                    <p className="lg:text-lg text-base font-medium  mb-4 text-black-100 capitalize">{backer?.backer_name}<span className='block text-sm font-normal'>
                 ...
                 <div className='flex flex-wrap mt-6'>
                  <button className='btn lg:mr-6 bg-transparent  h-10 min-h-10 lg:text-lg md:text-base text-xs
                    text-black-100 border border-custom-green-200'>See all</button>
                  <button className='btn bg-transparent h-10 min-h-10 lg:text-lg md:text-base  text-xs
                    text-black-100 border border-custom-green-200 mt-2 lg:mt-0'>See top donation</button>
                 </div>
                </div>
            </div>
    </div>
